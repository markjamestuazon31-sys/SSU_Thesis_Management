...
export function thesisTable(rows=[],opts={}){
  const {showOwner=false,showAdviser=false,showProgram=false,showYear=false,actionLabel='View record',actionRoute=(id)=>`/repository/${id}`}=opts;
  if(!rows.length)return emptyState('No thesis records found','Records matching this view will appear here.');
  return `<div class="table-wrap"><table class="data-table"><thead><tr><th>Title</th>${showOwner?'<th>Student / Researchers</th>':''}${showProgram?'<th>Course / Program</th>':''}${showYear?'<th>Research Year</th>':''}${showAdviser?'<th>Adviser</th>':''}<th>Status</th><th>Updated</th><th class="table-action-heading">Action</th></tr></thead><tbody>${rows.map(item=>{const label=typeof actionLabel==='function'?actionLabel(item):actionLabel;return `<tr><td><div class="table-title">${escapeHtml(item.title||'Untitled Thesis')}</div>${showProgram?'':`<div class="table-subtitle">${escapeHtml(item.program||'')}</div>`}</td>${showOwner?`<td><div>${escapeHtml(item.authors||item.studentName||item.ownerName||'—')}</div>${item.authors&&item.studentName?`<div class="table-subtitle">Submitted by ${escapeHtml(item.studentName)}</div>`:''}</td>`:''}${showProgram?`<td>${escapeHtml(item.program||'Not specified')}</td>`:''}${showYear?`<td><div>${escapeHtml(item.year||'—')}</div><div class="table-subtitle">${escapeHtml(item.academicYear||'')}</div></td>`:''}${showAdviser?`<td>${escapeHtml(item.adviserName||'Unassigned')}</td>`:''}<td>${statusBadge(item.status)}</td><td>${formatDate(item.updatedAt||item.createdAt)}</td><td class="table-action"><a class="table-record-action" href="#${actionRoute(item.id)}">${icon('eye',15)}<span>${escapeHtml(label)}</span></a></td></tr>`;}).join('')}</tbody></table></div>`;
}
