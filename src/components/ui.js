...
export function thesisTable(rows=[],opts={}){
  const {showOwner=false,showAdviser=false,actionLabel='View',actionRoute=(id)=>`/repository/${id}`}=opts;
  if(!rows.length)return emptyState('No thesis records found','Records matching this view will appear here.');
  return `<div class="table-wrap"><table class="data-table"><thead><tr><th>Title</th>${showOwner?'<th>Student</th>':''}${showAdviser?'<th>Adviser</th>':''}<th>Status</th><th>Updated</th><th></th></tr></thead><tbody>${rows.map(item=>`<tr><td><div class="table-title">${escapeHtml(item.title||'Untitled Thesis')}</div><div class="table-subtitle">${escapeHtml(item.program||'')}</div></td>${showOwner?`<td>${escapeHtml(item.studentName||item.ownerName||'—')}</td>`:''}${showAdviser?`<td>${escapeHtml(item.adviserName||'Unassigned')}</td>`:''}<td>${statusBadge(item.status)}</td><td>${formatDate(item.updatedAt||item.createdAt)}</td><td class="table-action"><a class="text-link" href="#${actionRoute(item.id)}">${escapeHtml(actionLabel)} ${icon('arrow',15)}</a></td></tr>`).join('')}</tbody></table></div>`;
}
