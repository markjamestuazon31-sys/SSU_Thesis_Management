...
import { isResearchInstructorRole } from '../../services/user.service.js';
...
export async function render({ profile }) {
  ...
  const roleText = profile.role === 'admin' ? 'Administrator' : isResearchInstructorRole(profile.role) ? 'Research Instructor' : profile.role === 'program_chair' ? 'Program Chair' : 'Student Researcher';
  ...
  return `<section class="workspace-hero workspace-hero-professional">
    <div class="dashboard-hero-content-pro">
      ...
      <h2 class="dashboard-hero-subtitle">${profile.role === 'admin' ? 'Thesis Administration Dashboard' : isResearchInstructorRole(profile.role) ? 'Research Instructor Dashboard' : profile.role === 'program_chair' ? 'Program Chair Dashboard' : 'Student Research Dashboard'}</h2>
      <p>${profile.role === 'program_chair'
        ? `Read-only program monitoring for ${escapeHtml(profile.program || 'your assigned program')}. Track Research Instructor-approved records through final administrator publication.`
        : `${escapeHtml(roleText)} workspace for thesis submissions, research workflow, repository access, approvals, and live monitoring.`}</p>
    ...
    <aside class="panel quick-panel">
      ...
      <div class="panel-body quick-list">
        ...
        <div class="notice-box live-notification-box">
          ...
          <p>${adminMode ? 'Workflow alerts, Research Instructor decisions, and thesis upload updates.' : 'Workflow alerts, Research Instructor feedback, and publication updates.'}</p>
...
function heroActions(role) {
  ...
  if (isResearchInstructorRole(role)) return `<a class="btn btn-primary" href="#/research-instructor/assigned">${icon('review', 17)} Review submitted research</a><a class="btn btn-secondary" href="#/research-instructor/history">${icon('clock', 17)} Review history</a>`;
  if (role === 'program_chair') return `<a class="btn btn-primary" href="#/program-chair/research">${icon('file', 17)} Monitor program research</a><a class="btn btn-secondary" href="#/program-chair/progress">${icon('clock', 17)} Research progress</a><a class="btn btn-secondary" href="#/program-chair/reports">${icon('chart', 17)} Program reports</a>`;
...
function workflowRoute(role) {
  ...
  if (isResearchInstructorRole(role)) return '/research-instructor/assigned';
  if (role === 'program_chair') return '/program-chair/progress';
...
function recentTable(rows, profile) {
  return thesisTable(rows, {
    ...
    showResearchInstructor: profile.role === 'admin' || profile.role === 'program_chair',
    statusLabels: profile.role === 'admin' ? { published: 'Uploaded Thesis' } : {},
    actionRoute: (id) => profile.role === 'student'
      ? `/student/thesis/${id}`
      : isResearchInstructorRole(profile.role)
        ? `/research-instructor/review/${id}`
        : profile.role === 'program_chair'
          ? `/program-chair/research/${id}`
          : `/admin/thesis/${id}`,
  });
}
...
function workflowSummary(theses = [], role) {
  const groups = role === 'admin'
    ...
    : role === 'program_chair'
      ? [
          ['Awaiting admin approval', ['instructor_approved', 'adviser_approved', 'recommended'], 'clock'],
          ['Admin approved', ['approved'], 'check'],
...
function quick(role) {
  ...
  if (role === 'program_chair') {
    return `<a class="quick-action" href="#/program-chair/research"><span class="quick-action-icon">${icon('file', 18)}</span><div><strong>Program research monitoring</strong><span>Open Research Instructor-approved records routed to your assigned program.</span></div>${icon('arrow', 16)}</a>
      <a class="quick-action" href="#/program-chair/progress"><span class="quick-action-icon">${icon('clock', 18)}</span><div><strong>Research progress</strong><span>Track each routed record through admin review and publication.</span></div>${icon('arrow', 16)}</a>
      <a class="quick-action" href="#/program-chair/reports"><span class="quick-action-icon">${icon('chart', 18)}</span><div><strong>Program reports</strong><span>View status, year, and Research Instructor monitoring summaries.</span></div>${icon('arrow', 16)}</a>`;
  }
  return `<a class="quick-action" href="#/admin/workflow"><span class="quick-action-icon">${icon('review', 18)}</span><div><strong>Manage workflow</strong><span>Final approve Research Instructor reviewed research and upload the thesis.</span></div>${icon('arrow', 16)}</a>
    <a class="quick-action" href="#/admin/users"><span class="quick-action-icon">${icon('users', 18)}</span><div><strong>Manage users</strong><span>Create Research Instructor and Program Chair accounts and control access.</span></div>${icon('arrow', 16)}</a>
